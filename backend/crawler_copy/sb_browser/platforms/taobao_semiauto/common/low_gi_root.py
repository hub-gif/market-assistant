...
def _market_assistant_root() -> Path:
    # common/ → taobao_semiauto → platforms → sb_browser → crawler_copy → backend → market_assistant
...
def tb_playwright_semiauto_capture_root() -> Path:
    """默认 Playwright 半自动落盘父目录（其下可按时间戳分子目录）。"""
    return (tb_data_dir() / "playwright_taobao_semiauto").resolve()
