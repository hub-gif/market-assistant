# -*- coding: utf-8 -*-
"""京东半自动（SeleniumBase）：CDP 监听；盘后见 ``jd_semiauto.postprocess``.

运行示例：``python -m sb_browser.platforms.jd_semiauto.seleniumbase_cdp.run_listen_demo``
"""
...
from . import flows as jd_flows
from .notes_sink_jd import JdCaptureSink

# Profile：留空=临时；绝对路径=持久登录；默认常量见 ``common.constants_jd_semiauto``。
SEMI_USER_DATA_DIR = ""
SEMI_HEADLESS = False
SEMI_SB_TEST_MODE = False

SEMI_LANDING_URL = ""           # 空则 SEMI_JD_DEFAULT_LANDING_URL
SEMI_CAPTURE_LABEL = "manual"

SEMI_SAVE_JSON = True
SEMI_SAVE_DIR = ""              # 空则 data/JD/sb_cdp_api_semiauto/
SEMI_SAVE_RUN_TIME_SUBDIR = True

SEMI_PAUSE_FOR_LOGIN_CONFIRM = True  # False=不阻塞直接监听
...
def _parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--stop-file",
        default="",
        help="停止信号路径；为空且提供 --run-dir 时默认 <run-dir>/.stop_requested",
    )
    p.add_argument(
        "--restart-file",
        default="",
...
def main() -> int:
    ...
    st = (args.stop_file or "").strip()
    if st:
        stop_file: Path | None = Path(st)
