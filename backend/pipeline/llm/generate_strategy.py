...
- **用户侧表述**：**禁止**虚构评价原文、访谈引语、带引号的「用户说…」；细则见下文「§2.1 痛点表证据信号」。
...
- **必须**把篇幅放在**策略**：痛点归纳、§2.5 动作表、怎么做与验证（后文章节）。

**数据与口径（硬性，与宿主分析报告同源输入）**：
- **§2.1 痛点表「证据信号」列（反捏造，硬性）**：
  - **禁止**书写「用户反馈『……』」「评价称『……』」等**带引号的逐字原话**，除非该片段在 `structured_brief`、`strategy_hints`、`report_strategy_excerpt` 或 `business_notes` 中**已出现相同或明显包含**的文本；否则一律**不得**用引号假装引用。
  - 若输入仅有主题级信号（关注词、负向归因方向、价差行数等），证据信号应写**可追溯归纳**，例如「与 brief 中 ×× 字段一致」「与报告第八章/节选已归纳的 ×× 主题一致」「监测摘要见 `strategy_hints` 第 n 条」，或写「**待原评论抽样核实**」——**禁止**把合理推测写成「用户已明确说……」的事实口吻。
...
- **成稿阶段避免**：反复「请业务决策」「待确认后再定」；不确定性一句带过即可。

**输出结构与阅读顺序（须与 `rules_draft_markdown` 章节一致，勿另起目录）**：
**摘要** → **一、顾客是谁**（含人群与路径、细类讨论、本品聚焦）→ **二、产品价值与用户痛点**（含痛点表、价值、对表、负向归因若有、**§2.5 策略动作总表**）→ **三、为什么要买「这款产品」** → **四、为什么要选「这个品牌」** → **五、与其它品牌有何不同** → **六、阶段目标与路径** → **七、品牌四线**（建设·打造·运营·体验）→ **八、战术支柱**（产品/定价/促销/渠道与传播）→ **九、风险、假设与待验证** → **十、下一步与节奏**（含业务备注）→ **附录**。
可微调小节标题用语，**不得**删减上述逻辑块或把「诊断数据」与「落地动作」顺序颠倒。

**语气**：面向业务读者，避免 CR1、心智等内部缩写；**勿在成稿中反复强调「对齐某报告第几章」**，以策略表述为主。

**策略表述硬性（痛点 → 怎么做，须覆盖全书，不得只写 §2～§8 部分章节）**：
- **总原则**：成稿**不是**第二份分析报告，也**不是**市场形势说明书。每条重要内容应能回答：**针对哪条用户痛点**（可与 §2.1 / §2.5 对应）、**我们采取什么动作**、**在具体触点怎么做**（商详/主图/短视频/客服/规格/价格呈现等）、**如何验证**（若适用）。
- **§2.5 策略动作总表**（若底稿已有表头）须**填写实质内容**；若无表，须在 **§二** 或 **§八** 用等价分条写清「痛点—动作—落地—验证」。

**分节要求（与底稿章节一一对应，勿省略）**：
- **摘要**：除范围样本外，**阶段重点**须含 1～2 条**可执行动作**，且能回扣 §2 中的优先痛点（非空泛「加强运营」）。
- **一、顾客是谁**：**禁止**重复报告中的细类词频、分品类样本量展开、文本挖掘方法；用 **少量结论句**（谁搜、关心什么）即可，**1.3 本品聚焦**须写清「本期主攻人群/场景」与 **§2 痛点**的对应关系。
- **二**：**§2.1 与 §2.4 须分工，避免「痛点」与「负向反馈」像两张重复表**：
  - **§2.1 痛点与证据**：面向**策略优先级**，用一行「痛点 | 证据信号 | 优先级」整合**全链路**（监测 + 评论侧均可作为证据信号），回答「**我们优先解决什么**」。此处**已**用负向主题作证据时，**不要**在 §2.4 再列同名清单。
  - **§2.4 负向评价主题归因**：只回答「**差评/负向评论在集中讨论哪些主题**」（与报告 §8/节选一致），是**评论侧归因视角**；**禁止**再复制 §2.1 的痛点行。**若**负向主题与 §2.1 高优痛点**完全对应**，§2.4 **允许且推荐**写 1～2 句互见：「负向评论主题与 §2.1 表中 P1/P2 同源，不重复展开」，**仅补充** §2.1 未单列的归因（如物流个案、某细类特有差评主题）。若 brief/节选已出现品牌或例句可作佐证，**禁止**编造未出现的品牌或原文。
  - §2.1 仍须**写全**监测已支撑的主要维度（口感/质地分线、分量/规格等，见下条），**证据信号列**遵守「§2.1 证据信号」硬性条款。§2.5 为全稿**动作总锚**，后文须能指回。
- **三**：§3.2 **每条转化障碍**须对应**至少一条应对动作**（谁、在什么页面/环节、改什么）；§3.1 时机叙述后须**承接** §2 中优先解决的痛点。
- **四**：品牌承诺与调性须能落到**可感知触点**（如商详第几屏、包装、客服首句），避免只有形容词。
- **五**：§5.2 差异化、§5.3 竞争应对须写清**相对竞品多做什么/少做什么、具体一步动作**。
- **六**：成功标准与 §6.2 路径须与 **§2.5** 动作**可对齐或合并叙述**；营销/总体策略句须为**动词导向**。
- **七**：品牌四线**每一条**至少一句：**服务哪类痛点、本周/本阶段具体做哪一步**。
- **八**：四支柱**每一支柱**须回扣 **痛点→动作→落地**（可与 §2.5 合并叙述，避免重复堆砌）。
- **九**：在表单风险勾选之外，**每条风险**尽量带**应对动作或验证计划**（抽样、核对规则），勿只列风险标题。
- **十**：下一步清单须为**可执行任务**（可含负责人/时间占位），与 §2.5 或 §六 优先级一致。
