...
- **禁止编造店铺结构**：不得写出「京东自营 SKU 占比」「自营超 X%」等**摘要中未给出**的定量句；店铺集中度仅可依据 `structured_brief.concentration` 与底稿，并区分列表行 / 去重 SKU 口径。
...
**第八章文本挖掘探针（当 JSON 中 `chapter8_text_mining_probe` 为真时，硬性）**：
- **禁止**将「关注词子串命中次数」「预设场景分组条数/占比」当作评论侧或需求侧的主要论据（`structured_brief` 已省略这些字段；与报告正文 §8 主口径一致）。
- 用户洞察、场景与传播结论须与宿主报告 **§8 文本挖掘** 及 `report_strategy_excerpt`（第九章）一致。
- **促销与活动**：若 `report_strategy_excerpt` 或 `structured_brief.price_promotion_signals` 中出现满减、券、到手价差、促销形态等，成稿须在**价格/营销/渠道**相关段落**承接**，与报告 **第六章、第九章** 建议一致，**不得无故省略**；**禁止**编造具体满减门槛或补贴比例（仅复述输入中已有线索）。

**输出结构与阅读顺序**：成稿建议保持与规则底稿一致：**一至十** 依次为——目标与边界 → 战场与样本 → 竞争与本品态势 → 价格带与定位 → 用户与评论侧 → 营销策略与总体方向 → 机会与 4P 支柱 → 风险 → 业务约束 → 下一步；可微调标题用语，**勿**把「诊断数据」与「落地动作」顺序颠倒。
