...
- **二**：§2.1 痛点表须**写全**监测已支撑的主要维度，至少覆盖：**口感/质地预期**（按细类分线，见下条）、**分量/规格/克重与场景是否匹配**（若摘要、负向主题或 `strategy_hints` 中出现该方向则**不可省略**）、信任与价格感知等；用**短语结论**，不写统计过程。**证据信号列遵守上文「§2.1 证据信号」硬性条款**，**禁止**为凑行数而捏造用户引语或未监测结论。§2.4 负向归因写**主题级结论**即可；若 brief/节选已出现具体品牌或例句可作**佐证**，**禁止**编造未出现的品牌或评价原文。§2.5 为全稿**动作总锚**，后文须能指回。
