...
- **本品事实（业务提供）**：若 `rules_draft_markdown` 的 **§1.3 本品聚焦** 下含 **业务侧本品依据** 段落：该段**仅**界定**本品侧**可引用的成分、功效表述、适用人群、规格与宣称边界，与监测数据**分源**。**成稿主线仍须**以 `structured_brief`、`report_matrix_group_evidence_md`、价带/矩阵等为**问题与机会依据**；**禁止**用大段照抄或扩写手册作为全文主体，**禁止**用本品依据**替代** §2.1 痛点表及评论证据链。**须结合写**：例如「节选/监测反映×× → 结合本品依据中可强调的×× → 本阶段拟××动作」，而非只展开产品说明。凡写本品事实性内容**须与该段落一致**；**禁止**编造明显矛盾；未提及的本品动作可用「拟/待业务确认」。**禁止**仅在文末或附录单独成章复述大块本品说明。竞品侧仍仅以 `structured_brief` 等为据。
...
**主次分源（硬性）**：全文主线必须是 **同任务竞品监测与报告同源数据**（`structured_brief`、`report_matrix_group_evidence_md`、价带/矩阵/评论归纳等）所支撑的**类目与用户侧结论、痛点与竞争格局**；底稿 §1.3「业务侧本品依据」仅提供**本品**成分、功效、人群与宣称**边界**类事实，用于**约束与补强**策略表述，**禁止**以手册或大段产品文案**代替**监测主线，**禁止**把成稿写成 primarily 产品介绍。**须综合**：先交代/沿用监测与节选中的依据，再衔接本品可怎么说、拟采取什么动作（合规且与数据同向）。
...
- **本品业务依据（硬性，与底稿 §1.3 一致）**：§1.3「业务侧本品依据」**不是**正文模板或成稿主干。**禁止**以大段复述手册代替 **§1.2 细类讨论、§2.1 与监测/节选**；**禁止**全文只见产品卖点而不见价带、评论主题与对抗逻辑。策略句应体现 **监测发现的问题或机会 + 本品在手册约束下可落地的说法或动作**（每条尽量能指回 brief 或节选）。手册中功效/成分表述仅可作**边界内引用**，须与 **竞品与用户侧证据** 编排成**策略**，而非连载说明书。
...
- **一、顾客是谁**：**禁止**重复报告中的细类词频、分品类样本量展开、文本挖掘方法；用 **少量结论句**（谁搜、关心什么、决策场景）；**1.3 本品聚焦**须写清本期主攻人群/场景/细类；若底稿含「业务侧本品依据」，**仅作与 §1.2/节选咬合一两句本品定位**，**禁止**在本节展开成大段产品说明书（**勿**写「与 §2.1 可对上」类作者提示）。**§1.2** **禁止**写入「类目结构（摘录）」式 SKU/条数表、「核心关注点」「高频词（含括号次数）」「共现词对」「主题归纳」及「依据 report_matrix_group_evidence_md」等**与竞品报告或矩阵节选同构**的字段化罗列；该等内容仅在报告与 JSON 节选，策略稿**至多两三句**定性差异即可，必要时一句「详见同任务《竞品分析报告》」带过。
...
STRATEGY_USER_PREFIX = (
    ...
    "输出前自检（本品依据 vs 监测）**：若底稿 §1.3 含「业务侧本品依据」，成稿是否**未**以手册扩写占满篇幅、**未**削弱 §2.1 与 `report_matrix_group_evidence_md` / `structured_brief` 的证据主线；是否写成「监测/节选 → 策略动作」，本品依据仅作**边界与结合点**？**若否**，先改再输出。\n"
