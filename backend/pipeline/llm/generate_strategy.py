...
**策略范围与前提（生成前先对齐）** → **摘要** → **一、顾客是谁**（含人群与路径、细类讨论、本品聚焦）→ **二、产品价值与用户痛点**（**仅 §2.1 针对痛点要怎么做** 表，**勿**再设独立「痛点表/价值对表/负向归因」子节）→ **三、为什么要买「这款产品」**（**仅 §3.1 为何要购买这一款（依据与理由）**；**无 §3.2**，转化与价带应对已在 §2 表内则**勿重复**）→ **四、为什么要选「这个品牌」** → **五、与其它品牌有何不同** → **六、阶段目标与路径** → **七、品牌四线**（建设·打造·运营·体验）→ **八、战术支柱**（产品/定价/促销/渠道与传播）→ **九、风险、假设与待验证** → **十、下一步与节奏**（含业务备注）→ **附录**。
...
- **三**：**仅 §3.1**，标题与底稿一致为**为何要购买这一款（依据与理由）**。**先**保留或转述输入中已有**检索/样本与价带**依据，**随后**用 1～2 句**直接**回答「用户为何要购买**这一款**（相对货架同类）」。**禁止**以「品类需求稳、概念认知成熟、适合××叙事切入」等**只适用于整个品类**的宏观句作为**唯一或最后**结论，而不落到**本品或主推规格**的可验证点（规格/配料/口感/价位等须与输入可对读）；宏观背景若写，**必须**收束到「因此用户更值得为这一款下单」的理由。可结合 brief 写价带锚点一句。**禁止**写 §3.2「转化障碍与应对」；若与购买相关的障碍与应对已在 §2.1 表内，§3.1 **勿再复述**。
