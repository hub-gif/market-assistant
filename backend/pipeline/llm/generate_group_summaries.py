...
``listing_snippets``（若干「标题｜规格:…｜标价｜券后｜详情价」摘录；**规格**优先来自「规格属性」列，若列不全则从**标题**中抽取的 ml、g、装量等片段，与合并表同源，已截断）。

请**为每个细类**输出一小段 Markdown（全部 groups 都要写，顺序与输入一致）：
- 以 ``#### `` + 与该 group 字段**完全一致**的细类名作为小节标题；
- 每段约 **80～200 字**中文，**只写价盘与价差**：用 ``price_stats`` 概括价带/离散度（如 min～max、中位数、相对集中或拉得开）；用 ``listing_snippets`` 归纳**标价 vs 券后 vs 详情价**是否常一致、是否常见券后低于标价、价差幅度的大致印象；**须结合**摘录中的 **「规格:」**、其同一行**标题**与规格片段说明**不同规格下绝对价是否可比**，**价高/价差大是否可能来自大规格或组合装**——仅当摘录里确有依据时写，勿展开成宣称解读；
