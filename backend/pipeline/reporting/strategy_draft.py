...
def build_strategy_draft_markdown(
    *,
    ...
    lines: list[str] = [
        ...
        "**全局禁止编造**：数字、品牌/店铺名、用户引语、活动规则等须能在 `structured_brief` / 报告节选 / 业务备注中核对；无依据处写「假设」「待核对」「输入未体现」（见 `generate_strategy.py` 中 `STRATEGY_DATA_RULES`）。",
        "",
    ]
    if generated_at_iso:
    ...
    lines.extend(
        [
            "### 1.3 本品聚焦（占位）",
            "",
            "*成稿写清本期**主攻人群/场景**与 §2「针对痛点要怎么做」的对应关系。*",
            "",
    ...
    lines.extend(
        [
            "## 二、产品价值与用户痛点",
            "",
            "*本节**仅**用下表写清**针对痛点要怎么做**（痛点简述 + 动作 + 落地 + 验证）。**不再**单设「痛点表 / 价值对表 / 负向归因」子节，避免与 §三、§八重复。*",
            "",
            ...
            "|------------------|----------|-----------------------------------|----------|",
            "| *（成稿依监测与评论归纳，可含口感/分量/价格信任等）* | *（动词句）* | *（可执行）* | *（指标或抽样）* |",
            "| | | | |",
            "",
        ]
    )
    ...
    if pst.get("n"):
        ...
        lines.append("")
    lines.append(
        "*成稿在 §3.1 末尾用 1～2 句**承接** §2 优先痛点并写购买理由；价带/规格类动作已在 §2 表内则此处**勿再展开一遍**。*"
    )
    lines.append("")
    ...
    lines.extend(
        [
            ...
            "### 6.2 路径",
            "",
            "*成稿：路径须与 **§2.1 针对痛点要怎么做** 可对齐；营销/总体策略为**动词句**，回扣痛点。*",
            "",
            _goal_bullet(
                "营销策略",
    ...
    lines.extend(
        [
            "## 八、战术支柱",
            "",
            "*成稿：四支柱分别回扣 **痛点→动作→落地**（可与 §2.1 呼应，避免纯重复）。*",
            "",
            "### 8.1 产品策略",
            "",
    ...
    lines.extend(
        [
            ...
            "## 十、下一步与节奏",
            "",
            "*成稿：下列为**可执行任务**（可补负责人/时间）；与 §2.1 / §六 优先级一致。*",
            "",
            "- [ ] 锁定主推款与对标；过法务与合规。",
            "- [ ] 统一对外数据口径与话术。",
            "- [ ] 下轮监测更新后迭代策略。",
